...
Perchè parliamo di learning non-supervisionato?

mod regressione <- "supervisionate", abbiamo var x, che pensiamo influenzino y

y <- "supervisiona" il modello, tramite val osservati posso avere un riscontro. 

LA VAR RISPOSTA "GUIDA" L'ANALISI

non-sup:

oss un set di variabili e cerco di capire:
* ...
* cerco cluster
* ...

NOTA: non ho nulla che "guida l'analisi"


### Nel dettaglio

* n oss. di p variabili $X_1, ... ,X_p$
* no var risposta
* studio "congiunto tra le var"
  * che relazioni?
  * ci sono var +- connesse?
  * classif. unità statistiche secondo valori


Tipicamente: si usa per fare analisi di tipo esplorativo

### Le tecniche

"di analisi multi-variata"

Vedremo due classi di metodi:

* analisi comp. principali
* analisi dei cluster e dei gruppi

## PCA (analisi comp. principali)

A partire da ins. con p variabili, voglio def. un ins. di var derivate chiamate COMPONENTI PRINCIPALI

CP <- voglio che riassumono gran parte variabilità insieme di partenza

es. uso: DS con p molto grande -> voglio costr. un DS con numero ridotto di variabili senza perdere troppe informazioni

* utile dal punto di vista della memoria
* utile dal punto di vista della sintesi di informazione

Appl. interessante: unire PCA e mod. regressione 

a volte, mi serve creare un mod. di regressione MA ho un numero molto elevato di variabili (rischio modello instabile)

SVANTAGGIO: perdiamo "interpretabilità", le var. derivate possono essere difficili da interpretare

VANTAGGIO: più semplice rapp. graficamente

### Come si procede

p variabili $X_1, ... ,X_p$, n osservazioni

Prima componente principale <- comb. lineare "normalizzata" delle var. di partenza che presenta varianza più alta

$Z_1 = ... $

$...$ <- "pesi" (sono normalizzati, la loro somma dei loro quadrati fa 1)

NOTA: $X_1, ... ,X_p$ potrebbero avere range, scale, distribuzioni, ecc... diversi (FENOMENI MOLTO DIVERSI) => è buona prassi standardizzare prima di procedere 

es. vogliamo set con varianza 1 e media 0

Ci sono varie procedure

Massimizzare la varianza (sotto vincolo normalizzazione)

Seconda, terza, ecc... comp. principale <- è la comb. lineare normalizzata che ha massima varianza ESCLUDENDO le comb. lineari di Z1

Z2 <- deve risultare ortogonale a Z1 (recupera l'informazione non presa dalla prima componente)
